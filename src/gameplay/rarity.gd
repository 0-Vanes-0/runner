...
static func generate_rarity() -> Rarity:
	var random_type: int = randi_range(1, CHANCES_SUM)
	if random_type <= NORMAL_CHANCE:
		return Rarity.new(NORMAL)
	elif random_type <= NORMAL_CHANCE + RARE_CHANCE:
		return Rarity.new(RARE)
	elif random_type <= NORMAL_CHANCE + RARE_CHANCE + EPIC_CHANCE:
